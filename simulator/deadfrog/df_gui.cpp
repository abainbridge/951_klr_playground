...
static char const *TextViewGetEndOfLine(char const *line) {
    char const *end_of_line = strchr(line, '\n');
    if (!end_of_line)
        end_of_line = line + strlen(line) + 1;
    return end_of_line;
}
...
void DfTextViewDo(DfWindow *win, DfTextView *tv, int x, int y, int w, int h) {
    ...
    for (line_num = 0; *line != '\0'; line_num++) {
        ...
        char const *end_of_line = TextViewGetEndOfLine(line);
    ...
    if (win->input.lmbClicked && mouseInRect && win->input.mouseY >= y) {
        tv->dragSelecting = true;
        tv->selectionEndX = tv->selectionStartX = 0;
        tv->selectionEndY = tv->selectionStartY = line_num;
    }

    // Update drag selection if mouse cursor is above or below the visible text.
    if (tv->dragSelecting) {
        if (win->input.mouseY < firstLineY) {
            tv->selectionEndX = 0;
            tv->selectionEndY = 0;
        }
        if (win->input.mouseY >= y) {
            tv->selectionEndX = 0;
            tv->selectionEndY = line_num;
        }
    }
...
char const *DfTextViewGetSelectedText(DfTextView *tv, int *numChars) {
    ...
    while (lineNum < selStartY) {
        char const *endOfLine = TextViewGetEndOfLine(line);
...
struct MenuItem {
    ...
    DfGuiAction m_action;
    int         m_top;

    bool IsSeparator() { return !m_action.menuItemLabel; }
};
...
struct Menu {
    ...
    void AddItem(DfGuiAction action) {
        if (m_numItems >= ARRAY_SIZE(m_items)) return;
        ...
        if (action.shortcut.key) {
            char buf[64] = "";
            if (action.shortcut.ctrl) strcat(buf, "Ctrl+");
            if (action.shortcut.shift) strcat(buf, "Shift+");
            if (action.shortcut.alt) strcat(buf, "Alt+");
            strcat(buf, GetKeyName(action.shortcut.key));
    ...
    void Render(DfWindow *win) {
        ...
        for (int i = 0; i < m_numItems; ++i) {
            ...
            if (item->IsSeparator()) {
                ...
                char const *label = item->m_action.menuItemLabel;
                DrawTextSimple(g_defaultFont, g_normalTextColour, win->bmp,
                    x + 10, textY, label);

                if (item->m_action.hotKeyIndex >= 0) {
                    int offset = GetTextWidthNumChars(g_defaultFont, label, item->m_action.hotKeyIndex);
                    int len = GetTextWidthNumChars(g_defaultFont, label + item->m_action.hotKeyIndex, 1);
    ...
    int GetWidth() {
        ...
        for (int i = 0; i < m_numItems; i++) {
            MenuItem *item = m_items[i];
            int width = GetTextWidth(g_defaultFont, item->m_action.menuItemLabel);
...
//         }
    ...
    MenuItem *FindMenuItemByHotkey(char c) {
        ...
        for (int i = 0; i < m_numItems; i++) {
            MenuItem *item = m_items[i];
            if (item->m_action.hotKeyIndex >= 0) {
                char hotkey = tolower(item->m_action.menuItemLabel[item->m_action.hotKeyIndex]);
                if (hotkey == c) {
                    return item;
                }
            }
    ...
    DfGuiAction CheckKeyboardShortcuts(DfWindow *win) {
        for (int i = 0; i < m_numMenus; i++) {
            ...
            for (int j = 0; j < menu->m_numItems; j++) {
                ...
                if (win->input.keyDowns[item->m_action.shortcut.key] &&
                    win->input.keys[KEY_CONTROL] == item->m_action.shortcut.ctrl &&
                    win->input.keys[KEY_SHIFT] == item->m_action.shortcut.shift &&
                    win->input.keys[KEY_ALT] == item->m_action.shortcut.alt) {
                    return item->m_action;
                }
            }
        }
...
//              ClearAllState();
    ...
    DfGuiAction Advance(DfWindow *win) {
        ...
        if (m_contextMenu) {
            m_contextMenu->CalculateScreenPostions(100, 100);
...
void DfMenuBarAddAction(DfMenuBar *dfMb, char const *menuName, 
        char const *menuItemLabel, int hotKeyIndex, DfKeyboardShortcut shortcut) {
    ...
    if (menu)
        menu->AddItem({ menuName, menuItemLabel, hotKeyIndex, shortcut });
}
...
void DfMenuBarShowContextMenu(DfMenuBar *dfMb, DfContextMenu *dfCm, int x, int y) {
    MenuBar *mb = (MenuBar*)dfMb->internals;
    Menu *cm = (Menu *)dfCm->internals;
    mb->ShowContextMenu(cm);
}


DfContextMenu DfContextMenuCreate() {
    return { new Menu("Context") };
}


void DfContextMenuAddAction(DfContextMenu *cm, char const *menuItemLabel, int hotkeyIndex) {
    Menu *menu = (Menu *)cm->internals;
    menu->AddItem({ "Context", menuItemLabel, hotkeyIndex, {} });
}
